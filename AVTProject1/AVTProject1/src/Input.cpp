...
namespace AVTEngine
{
	...
	void Input::keyPressCallback(unsigned char key, int x, int y)
	{
		...
		// change camera
		case '1':
			Application::Instance->changeCamera(1);
			break;
		case '2':
			Application::Instance->changeCamera(2);
			break;
		case '3':
			Application::Instance->changeCamera(3);
			break;
		}
	}
	...
	void Input::mouseMovementCallback(int x, int y)
	{
		...
		Application::Instance->getCurrentCamera().setArcballOffset({ offsetX, offsetY, offsetZ });
	}
